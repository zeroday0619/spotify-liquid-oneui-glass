...
def discover_config(executable):
    """Use the CLI's active configuration as the authority for automatic apply."""
...
def default_config():
    """Provide an XDG fallback for inspection and installation without apply."""
    xdg = os.environ.get('XDG_CONFIG_HOME')
    if xdg and Path(xdg).is_absolute():
        base = Path(xdg)
    else:
        base = Path.home() / '.config'
    return base / 'spicetify'


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('action', choices=('install', 'uninstall'), nargs='?', default='install')
    parser.add_argument('--config-dir', type=Path, help='Initialized configuration directory (CLI path, then XDG_CONFIG_HOME/spicetify or ~/.config/spicetify)')
    ...
    try:
        ...
        discovered = discover_config(args.spicetify)
        config = args.config_dir
        if config is None:
            config = discovered or default_config()
        config = config.expanduser().resolve()
        if not args.no_apply and not args.dry_run and config != discovered:
            raise ValueError('The selected directory differs from the CLI configuration or its active path could not be verified. Use --no-apply and configure Spicetify before applying.')
